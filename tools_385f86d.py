...
from qwen_mcp.client import DashScopeClient, registry
...
{{
  "manifest": "Technical recursion of the goal",
...
async def generate_audit(content: str, context: Optional[str] = None, ctx: Optional[Context] = None) -> str:
    ...
    async def report_progress(delta: str, total_len: int):
        if ctx and total_len % 50 == 0:
            await ctx.report_progress(progress=total_len, total=None, message=f"Auditing... ({total_len} chars generated)")
...
async def generate_code(prompt: str, context: Optional[str] = None, ctx: Optional[Context] = None) -> str:
    ...
    async def report_progress(delta: str, total_len: int):
        if ctx and total_len % 50 == 0:
            await ctx.report_progress(progress=total_len, total=None, message=f"Coding (Qwen-3.5-Plus)... ({total_len} chars generated)")
...
async def generate_code_25(prompt: str, context: Optional[str] = None, ctx: Optional[Context] = None) -> str:
    ...
    async def report_progress(delta: str, total_len: int):
        if ctx and total_len % 50 == 0:
            await ctx.report_progress(progress=total_len, total=None, message=f"Coding (Qwen-2.5-Coder)... ({total_len} chars generated)")
...
async def generate_lp_blueprint(goal: str, context: Optional[str] = None, ctx: Optional[Context] = None) -> str:
    ...
    if ctx:
        await ctx.report_progress(progress=0, total=None, message="[Phase 1] Discovery: Hiring expert squad...")
    ...
    try:
        ...
        discovery = {"project_name": "LP Task", "hired_squad": [{"role": "Generalist", "audit_filter": "Sanity check"}]}

    squad_str = ", ".join([f"{m['role']} ({m['audit_filter']})" for m in discovery.get("hired_squad", [])])

    # INTERNAL SELF-HEALING LOOP
    while attempt < max_retries:
        if ctx:
            await ctx.report_progress(progress=0, total=None, message=f"[Phase 2] Architecting (Attempt {attempt+1}/{max_retries})...")
            
        # PHASE 2: Architecting
        user_content = f"Project: {discovery.get('project_name')}\nTarget Goal: {goal}\n\n"
        if context:
        ...
        async def architect_progress(delta: str, total_len: int):
            if ctx and total_len % 50 == 0:
                await ctx.report_progress(progress=total_len, total=None, message=f"[Phase 2] Drafting Blueprint... ({total_len} chars)")
        
        blueprint_json_raw = await client.generate_completion(
            messages=architect_messages,
            temperature=0.2,
            task_type="strategist" if attempt == 0 else "coding", # Use smarter model on retry
        ...
        try:
            ...
            if ctx:
                await ctx.report_progress(progress=0, total=None, message=f"[Phase 3] Self-Verification in progress...")
            ...
            try:
                ...
                if not verification.get("is_valid", True):
                    last_error = verification.get("structural_fix", "Unknown degeneration logic.")
                    attempt += 1
...
async def list_repo_files(directory: str = ".", pattern: str = "**/*") -> str:
    ...
    try:
        ...
        for p in root.rglob("*"):
            if p.is_file():
                # Ignore common noise
                if any(x in str(p) for x in [".git", "__pycache__", ".venv", "node_modules"]):
                    continue
...
async def list_available_models() -> str:
    ...
    from qwen_mcp.client import DashScopeClient
    client = DashScopeClient()
...
async def set_model_in_registry(role: str, model_id: str) -> str:
    ...
    from qwen_mcp.client import registry, DashScopeClient
