# 🛠️ Qwen HUD: Kompilacja i Pakowanie
...
Musisz najpierw zbudować interfejs UI, który znajduje się w podkatalogu `qwen-hud-ui`. Wynik trafi do `vscode-extension/dist`.
...
1.  **"npm error Missing script: build"**: Uruchamiasz komendę w złym katalogu. Pamiętaj, że `npm run build` działa TYLKO w `qwen-hud-ui`.
