...
from qwen_mcp.tools import (
    generate_audit, generate_code, generate_code_25, 
    generate_lp_blueprint, read_repo_file, list_repo_files, 
    generate_usage_report, list_available_models, set_model_in_registry
)
...
@mcp.tool()
async def qwen_audit(content: str, context: Optional[str] = None, ctx: Context = None) -> str:
    """
...
@mcp.tool()
async def qwen_coder(prompt: str, context: Optional[str] = None, ctx: Context = None) -> str:
    """
...
@mcp.tool()
async def qwen_coder_25(prompt: str, context: Optional[str] = None, ctx: Context = None) -> str:
    """
...
async def qwen_architect(goal: str, context: Optional[str] = None, ctx: Context = None) -> str:
    ...
    """
    if ctx:
        await ctx.report_progress(progress=0, total=None, message="Initiating Lachman Protocol...")
